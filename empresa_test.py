    ...
    for teste in testes:
        try:
            ...
            assert not isinstance(
                teste, int), 'Levantou TypeError para entrada inteira'
        except ValueError:
            assert isinstance(
                teste, int), 'Levantou ValueError para entrada que não é inteira'
            assert teste <= 0, 'Levantou ValueError para entrada válida'
        except Exception:
            raise AssertionError('Não levantou o erro do tipo pedido')
        else:
            raise AssertionError(
                'Não levantou nenhum erro para uma entrada inválida')
...
def test_03_inclui_func():
    ...
    assert len(
        funcinarios) == 3, 'Lista de funcionários não contém o número correto de funcionários contratados'
